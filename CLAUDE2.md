...
> **Draft — for review.** This replaces CLAUDE.md for the versioned (v0–v9) workflow described in `BLUE_PRINT2.md`. Same rules and discipline as before; the only real change is what a "session" produces.
...
**Full spec lives in `BLUE_PRINT2.md`** — the versioned build plan, schema, and API contract are there. CLAUDE2.md is the workflow guide; BLUE_PRINT2.md is the source of truth for what we're building and in what order.
...
```
TripFlow/
├── CLAUDE2.md              ← this file (workflow guide)
├── BLUE_PRINT2.md          ← full versioned spec (read this for decisions)
├── FUTURE_SCOPE2.md        ← things genuinely out of scope past v9
...
See `BLUE_PRINT2.md` Section 3 for the full detail per version.
...
**Never silently diverge from BLUE_PRINT2.md.**
If something in BLUE_PRINT2.md conflicts with what you're about to build, flag it explicitly and ask how to resolve it. Do not change the approach silently.
...
See `BLUE_PRINT2.md` Section 7 for the full list with version tags. Summary:
...
See `FUTURE_SCOPE2.md` — multi-city trips, editable/draggable plans, multi-user/shared trips, city-level place caching, notifications, mobile app. Everything else discussed for this project has a version number in `BLUE_PRINT2.md`.
